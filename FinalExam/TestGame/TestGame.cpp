...
void drawLine(float x0, float y0, float x1, float y1, const char* symbol) {
    ...
    while (1) {
        screen[(int)roundf(y0)][(int)roundf(x0 * 2)] = symbol[0];
        screen[(int)roundf(y0)][(int)roundf(x0 * 2 + 1)] = symbol[1];
...
// 삼각형 그리기 함수
void drawTriangle(float x1, float y1, float x2, float y2, float x3, float y3, const char* symbol) {
    drawLine(x1, y1, x2, y2, symbol);
    drawLine(x2, y2, x3, y3, symbol);
    drawLine(x3, y3, x1, y1, symbol);
}
...
void GameLoop() {
    while (1) {
        ...
        drawTriangle(cx, cy - 8.0f, cx - 10.0f, cy + 8.0f, cx + 10.0f, cy + 8.0f, "@@");
